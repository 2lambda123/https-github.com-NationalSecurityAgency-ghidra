...
public class TraceDisassembleCommand
  ...
  protected Disassembler getDisassembler(TraceProgramView view,
                                         TaskMonitor monitor) {
    return Disassembler.getDisassembler(platform.getLanguage(),
                                        platform.getAddressFactory(), monitor,
                                        monitor::setMessage);
  }

  protected MemBuffer getBuffer(TraceProgramView view) {
    return platform.getMappedMemBuffer(view.getSnap(),
                                       platform.mapHostToGuest(start));
  }
  ...
  protected AddressSetView writeBlock(TraceProgramView view,
                                      InstructionBlock block) {
    InstructionSet set = new InstructionSet(platform.getAddressFactory());
    set.addBlock(block);
    return view.getTrace().getCodeManager().instructions().addInstructionSet(
        Lifespan.nowOn(view.getSnap()), platform, set, true);
  }
  ...
  public boolean applyToTyped(TraceProgramView view, TaskMonitor monitor) {
    ...
    InstructionBlock block =
        disassembler.pseudoDisassembleBlock(buffer, initialContext, limit);
    if (block == null) {
  ...
  public AddressSetView getDisassembledAddressSet() { return disassembled; }
}
